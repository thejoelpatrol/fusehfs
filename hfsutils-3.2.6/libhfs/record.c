...
void r_packdirent(CatDataRec *data, const hfsdirent *ent)
{
  switch (data->cdrType)
    ...
    case cdrDirRec:
	  ...
	  data->u.dir.dirFndrInfo = *(DXInfo*)ent->u.dir.xinfo;
      break;
	...
	d_getsl((const unsigned char *) ent->u.file.creator);
	  ...
	  data->u.fil.filUsrWds.fdFldr = ent->u.file.window;
	  data->u.fil.filFndrInfo = *(FXInfo*)ent->u.file.xinfo;
      break;
    }
}
		    ...
		    const CatDataRec *data, hfsdirent *ent)
{
  ...
  switch (data->cdrType)
    ...
    case cdrDirRec:
	  ...
	  *(DXInfo*)ent->u.dir.xinfo = data->u.dir.dirFndrInfo;
    ...
    case cdrFilRec:
	  ...
	  ent->u.file.window = data->u.fil.filUsrWds.fdFldr;
			
	  *(FXInfo*)ent->u.file.xinfo = data->u.fil.filFndrInfo;
